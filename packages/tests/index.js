...
function getPackageList() {
  const lernaConfig = require('../../package.json');

  return lernaConfig.workspaces
    .filter(pckg => pckg.indexOf('packages/benchmarks/') === 0)
    .map(pckg => {
      // NOTE: use NODE_PATH env variable to prevent .. .. ..
      const { name, benchmarks } = require(path.join(
        '..',
        '..',
        pckg,
        'package.json'
...
async function run() {
  ...
  for (let i = 0; i < packages.length; i++) {
    ...
    if (!process.env.SKIP_BUILD) {
      ...
      console.log('');
      shell.exec(
        `yarn lerna run --loglevel=silent --scope ${packageInfo.module} build`
      );
